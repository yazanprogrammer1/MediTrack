...
import com.example.meditrack.ui.theme.*
...
fun CustomSwitch(
    ...
    val thumbPaddingStart =
        (MaterialTheme.dimens.appDrawerDimens.switcherDimens.switchHeight - uncheckedThumbDiameter) / 2
    val minBound = with(LocalDensity.current) { thumbPaddingStart.toPx() }
    val maxBound =
        with(LocalDensity.current) { MaterialTheme.dimens.appDrawerDimens.switcherDimens.thumbPathLength.toPx() }
    ...
    Box(
        modifier
            ...
            .requiredSize(
                MaterialTheme.dimens.appDrawerDimens.switcherDimens.switchWidth,
                MaterialTheme.dimens.appDrawerDimens.switcherDimens.switchHeight
            )
    ) {
        SwitchImpl(
            checked = checked,
